...
export async function GET(_:Request,{params}:{params:Promise<{slug:string}>}){
 ...
 try{
  ...
  const players=race.participants.map(p=>p.player).sort((a,b)=>b.lp-a.lp).map((p,i)=>({position:i+1,riotId:p.riotId,region:p.region,rank:p.rank||'Unranked',lp:p.lp,wins:p.wins,losses:p.losses,peakLp:p.peakLp,winrate:p.wins+p.losses?Math.round(p.wins/(p.wins+p.losses)*100):0,updatedAt:p.updatedAt}));
