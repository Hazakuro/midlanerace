...
export default function RaceLive({slug}:{slug:string}){
 ...
 return <div>
  {loading&&!data?<div className="card">Загрузка гонки…</div>:error?<div className="card error">{error}</div>:<>
   ...
   <div className="stats"><div><span>Участники</span><b>{total}</b></div><div><span>Лидер</span><b>{leader?`${leader.rank} · ${leader.lp} LP`:'—'}</b></div><div><span>Матчи</span><b>{updated}</b></div><div><span>Последнее обновление</span><b>{lastUpdate?new Date(lastUpdate).toLocaleTimeString('ru-RU'):'—'}</b></div></div>
   <div className="card tableCard"><div className="tableTitle"><b>LEADERBOARD</b><span className="muted">по рангу и LP</span></div><div className="tableWrap"><table><thead><tr><th>#</th><th>Игрок</th><th>Ранг</th><th>LP</th><th>W/L</th><th>Матчи</th><th>WR</th><th>Peak</th></tr></thead><tbody>{data.players.map((p:any)=><tr key={p.riotId}><td className="pos">{p.position}</td><td><Link className="playerLink" href={`/players/${encodeURIComponent(p.riotId)}`}>{p.riotId}</Link><small>{p.region}</small></td><td><div style={{display:'flex',alignItems:'center',gap:10}}><RankIcon rank={p.rank} size={34}/><span>{p.rank}</span></div></td><td><strong>{p.lp}</strong></td><td>{p.wins} / {p.losses}</td><td><strong>{p.totalMatches}</strong></td><td>{p.winrate}%</td><td>{p.peakLp}</td></tr>)}</tbody></table></div></div>
